...
TypeScript and more. Finally, we extensively studied C++ as its templates 
engine bears strong resemblance to refinement types and we observe its limitation. 
...
or impossible to decide statically. 
...
\par
Consider three classes \verb|A|, \verb|B extends A| and 
\verb|C extends B|. The program \verb|A x = new B();(C) x| 
  ...
  \begin{center}
    \begin{tabular} {c}
      \inference {\Gamma \vdash \upsilon : T & \Gamma, \upsilon : T \vdash e : bool}
        {\Gamma \vdash \{\upsilon : T\text{ }|\text{ }e\}: Type}[(Refinement Type formation)]
    \end{tabular}
  \end{center}
...
how these refinement types are validated dynamically at runtime. 
...
refinement type, the evaluation context is retrieved from $\mathcal{R}$, the 
...
Recall that previous we proposed Simple-$R_{C}$ which uses closures to handle 
...
showed how our variant avoid these pitfalls by disallowing referencing. We 
