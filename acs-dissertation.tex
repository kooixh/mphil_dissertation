  ...
  \begin{center}
    \begin{tabular} {c}
      ...
      \inference {\Gamma \vdash e_1: T_1 & \Gamma \vdash e_2: T_2} 
        {\Gamma \vdash \textbf{let } x = e_1 \textbf{ in } e_2: T_2} [(\textbf{let in})] \text{ }
    \end{tabular}
  \end{center}
...
e$.
  ...
  the formuli. 
\end{itemize}
We first define the operational semantics for the basics expression in 
Simple-$\Pi$ for the basic configuration. A configuration is defined as a 
pair $\langle e, \sigma \rangle$, an expression $e$ and value context $\sigma$.
...
\begin{figure}[H]
  \begin{center}
    \begin{tabular} {c}
      ...
      & \\
      \inference {n = n_1 + n_2}{\langle n_1 + n_2, \sigma \rangle \longrightarrow \langle n, \sigma\rangle}[(op +)] \text{ }
      \inference {n = n_1 - n_2}{\langle n_1 - n_2, \sigma \rangle \longrightarrow \langle n, \sigma\rangle}[(op -)]
      & \\
      \inference {b = b_1 \wedge b_2}{\langle b_1 \wedge b_2, \sigma \rangle \longrightarrow \langle b, \sigma \rangle}[(op $\wedge$)] \text{ }
      \inference {b = b_1 \vee b_2}{\langle b_1 \vee b_2, \sigma \rangle \longrightarrow \langle b, \sigma \rangle}[(op $\vee$)]
      & \\
      \inference {\langle e_1, \sigma\rangle \longrightarrow \langle e'_1, \sigma\rangle & 
...
scope, i.e. the configuration on top of $\rho$, which will be removed 
subsequently.
...
We define the behaviour of scopes using the following evaluation rules. 
\begin{figure}[H]
  \begin{center}
    \begin{tabular}{c}
      \inference {\sigma' = \sigma + \{x \mapsto v\} & \rho' = push(\rho, \langle \text{\textbf{skip}}, \sigma \rangle)} 
      ...
      & \\
      \inference {\sigma' = \sigma + \{x \mapsto v\} & \rho' = push(\rho, \langle e_2, \sigma \rangle)} 
...
Functions in almost all imperative language are in their own scope. In many 
cases they do not have the access to the variables out of their scope however 
  ...
  \begin{center}
    \begin{tabular} {c}
      \inference {((x_1..x_n), e_{body}) = \textit{fns}(f) 
        \\ \sigma' = \{x_1 \mapsto v_1 ... x_n \mapsto v_n\} & \rho' = push(\rho, \sigma)}
...
\begin{figure}[H]
  \begin{center}
   \begin {tabular} {c} 
     \inference {\Gamma \vdash x: T & \Gamma \vdash e_1: T \\ \Gamma \vdash \forall \alpha : \Pi x. P(x)\text{ }s.t.\text{ }\alpha : \Pi e_1. P(e_1)} 
          {\Gamma \vdash x := e_1 : T} [($:=$)] \text{ }
    \end{tabular}
  \end{center}
