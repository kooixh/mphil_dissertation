...
they fail to caputure how these types systems could relate to 
...
\par
In chapter \ref{chapter:background} we provide background on the areas, we first give the 
...
\par
In chapter \ref{chapter:related_work} we will discuss our work with regards to the 
...
satify \verb|P| or it gives the value \verb|v| which is refined using \verb|P|. 
...
\par
C++ 11 introducted the notion of 
...
Throughout our research, we observed certain ideas that are commonly used across 
different work. In this next chapter, we will introduce key concepts that are 
relevant to our project and how these idea will be used to create a refinement 
type system.

\section{Immutability vs Constant} \label{section:const_immutable}
...
immutability however they are different concepts. A \textit{constant} is a 
...
\begin{figure}[H]
  \begin{center}
    ...
    \end{tabular}
  \end{center}
  \caption{Modifying Constant in Java}
...
\par
Similarly in C++, consider the code in figure \ref{code:cpp_const} where one declare a pointer \verb|p| 
that points to a constant integer. C++ ensures that one is not able to modify the 
...
variables in the outer scope even if the variables has gone out of scope. 
...
\par
Semantically, a closures can be modelled as a data structure mapping 
...
useful. The ability to bind values to variables within a scope will be explored 
further in our project.

\section{Hybrid Type Checking}
...
\begin{itemize}
  ...
  \item The \textit{Cmd} type to represent a command.
\end{itemize}
...
return keyword is ommited. Functions return can be difficult to represent, in many 
...
\begin{figure}[H]
  \begin{center}
    \begin{tabular} {c}
      ...
      \inference {\Gamma \vdash e_1: bool \\ \Gamma \vdash C_1: Cmd & \Gamma \vdash C_2: Cmd}
        {\Gamma \vdash_{C} \text{\textbf{if }} e_1 \text{\textbf{ then }} 
        C_1 \text{\textbf{ else }} C_2: Cmd}[(\textbf{if else})]
      \inference {\Gamma \vdash e_1: bool & \Gamma \vdash C_1: Cmd}
        {\Gamma \vdash_{C} \text{\textbf{while }} e_1 \text{\textbf{ do }} C_1 : Cmd} [(\textbf{while do})] & \\
      \inference {\Gamma \vdash x: T & \Gamma \vdash e_1: T} 
        {\Gamma \vdash_{C} x := e_1 : Cmd} [($:=$)] \text{ }
      \inference {\Gamma \vdash C_1: Cmd & \Gamma \vdash C_2: Cmd} 
        {\Gamma \vdash_{C} C_1;C_2 : T_2} [(concatenation $;$ $C$)] \text{ }
      & \\
      \inference {\Gamma \vdash e_1: T_1} 
        {\Gamma \vdash_{D} \textbf{var } x : T_1 = e_1 \dashv \Gamma + \{x \mapsto T_1\}} [($vars$ Declaration)]
      & \\
      \inference {\Gamma \vdash D_1 \dashv \Gamma' & \Gamma' \vdash_{C} C_1 : Cmd} 
...
\par
We define an unsound typing judgement to capture this behaviour statically. 
\begin{figure}[H]
  \begin{center}
    \begin{tabular} {c}
...
statically. Gordon Plotkin, et al discussed the idea of dynamically checking a static 
...
\par
The refinement predicate in general proves to be undecideable 
at compile time, one might require access to program state for values 
of variables in order to evaluate the refinement predicate. In many cases, 
...
\par
Previously we demonstrated how basic properties of refinement types are checked 
statically in Simple-$R$ using an unsound judgement, now we show 
how thsee refinement types are validated dynamically at runtime. 
We define a special form of typing judgement to type check refinement types dynamically.
...
not able to use \textbf{typedef} to provide an alias for a base type, i.e. 
$\textbf{typedef } x = int$ is forbidden. 
...
varaible is a value of a type. 

\par
The typing judgement has to be updated to accomodate type checking using a 
declared type. All typing judgement are updated such that $\tau$ is accessible.

\begin{center}
  \begin{tabular} {c}
    \inference{\Gamma \vdash \{\upsilon : T\text{ }|\text{ }e\} : Type}
    {\Gamma, \tau \vdash_{D} \textbf{typedef } x = \{\upsilon : T\text{ }|\text{ }e\} 
    \dashv \Gamma + \{x \mapsto Type\}, \tau + \{x \mapsto \{\upsilon : T\text{ }|\text{ }e\}\}}  
  \end{tabular}
\end{center}
...
\begin{center}
  \begin{tabular} {c}
    \inference {\Gamma \vdash e_1: T_1} 
        {\Gamma \vdash_{D} \textbf{const } x : T_1 = e_1 \dashv \Gamma + \{x \mapsto T_1\}} [($vars$ Declaration)]
  \end{tabular}
\end{center}
...
defining the variables immutable. We define a new variant of Simple-$R$ named 
Simple-$R_{C}$ that extends the former utilises this mechanic for handling 
refinement types. 
...
and named the approach \emph{Type Closure}.
...
\par
Our appraoch is to have the predicate to evaluate in a closure environment, we 
...
closure for named type, if one declares a refinement type inline the rule 
(Refine Runtime) that evaluates the predicate using the environment still hold. 
...
snapshotting the value of variabels at the time of type definition. 
...
not accomodate reference variables and pointers. If a refinement type 
relies on pointers in the refinement predicate then the simple act of declaring 
them immutable is not enought. Pointer values can be altered through aliasing. 
...
we have omited the need for manual memory management and we 
...
\begin{figure}[H]
  \begin{center}
    \begin{tabular} {c}
      ...
      & \\
      \inference {\Gamma \vdash e_2 : T & \Gamma \vdash e_1 : T\textit{ ref}} {\Gamma \vdash_{C}\text{ }!e_1 := e_2 : Cmd}
    \end{tabular}
  \end{center}
...
\par
The program is valid because in C/C++ the keyword \textbf{const} does not mean 
immutable, instead the keyword simply provides a contract that they variable 
will not be used to modify the value, it does not guarantee that the values 
would not be change through a different manner. 
...
Recall that previous we propsed Simple-$R_{C}$ which uses closures to handle 
...
showed how our variant avoid these pitfalls by disallowing referncing. We 
conclude the chapter by extending Simple-$R_{I}$ and Simple-$R_{C}$ with our new  
idea. 

\chapter{Related Work} \label{chapter:related_work}

\chapter{Further Work} \label{chapter:further_work}
