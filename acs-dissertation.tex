...
the same support. Research has attempted to bring dependent types to imperative 
programming and while they have been successful in the project, the languages 
that are created is not widespread. 

\par
Informally, one is able to implement refinement types support in 
different mainstream languages such as Scala and C++ however they are restricted 
and not documented. We analyse \textit{refined}, a Scala based library that 
introduces refinement types to the language. Finally, we attempted to construct 
refinement types in C++ using templates. 

\par
Throughout our research, we observed certain ideas that are common across 
different work. In the next chapter, we will introduce key concepts that are 
...
approach where we \emph{bind free variables with a value} and named the approach 
\emph{Type Closure}.
