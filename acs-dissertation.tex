...
\begin{figure}[H]
  \begin{center}
    \begin{tabular} {l l l}
      ...
      vals$ \\
      \textit{Type Variable Context} & $\tau$& $:$ $vars \mapsto T$ \\
    \end{tabular}
  \end{center}
...
\par 
We use the notation $P[x/a]$ to denote in the function $P$, replace all 
...
\begin{figure}[H]
  \begin{center}
    ...
    y = \Pi x : A. P(x)\{P =  P[x/\alpha] \wedge y \in \text{\textbf{dom}}(\tau)\}$ 
    \normalsize
  \end{center}
...
that we are substituting a variable with a value, essentially fixing the value 
