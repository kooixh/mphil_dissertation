...
This chapter will introduce the Simple-$\Pi$ language. The language will follow 
the imperative paradigm, specifically a procedural language. Most importantly, 
the language is statically typed with support for dependent typing. 

\section{Language Syntax}
This section describes the syntax of the Simple-$\Pi$ language. 
...
\begin{figure}[H]
  \begin{center}
    \begin{tabular}{l l l}
      ...
      \textit{Expressions} & $e$ & $:$ \textbf{skip} $|$ $x$ $|$ $x\text{ }:= e$ $|$ $e_1 + e_2$ $| \text{ }e_1 - e_2$ $|$ $e_1 \wedge e_2$ $|$ $e_1 \vee e_2$ \\
      & & \; $|$ \textbf{if} $e_1$ \textbf{then} $e_2$ \textbf{then} $e_3$ $|$ \textbf{while} $e_1$ \textbf{do} $e_2$ $|$ 
      $\text{\textbf{call}}(\lambda_1, e)$\\
      \textit{Variables} & $vars$& $:$ $x \in {a,b,c...z}$\\
      \textit{Values} & $vals$& $:$ $\mathbb{Z}$ $|$ $\mathbb{B}$ $|$ $(e, e')$ $|$ $\{a\text{ }:
      \text{ }T\text{ }|\text{ } P(a)\}$\text{ }$|$\text{ }$\lambda x.e$\\
      \textit{Context} & $\sigma$& $:$ $vars \mapsto vals$
    \end{tabular}
  \end{center}
  \caption{Syntax for Simple-$\Pi$ language}
\end{figure}

\par
The Simple-$\Pi$ language has support for five different data types. In addition to 
supporting the basic integers and boolean, it has the notion of a pair 
($T \times T$), dependent types ($\Pi x: A.P(x)$) and the \textit{void} type to 
represent null or empty. 

\par
Notice that in the definition functions are defined as taking a type and 
returning a type. As the language supports the notion of pairs, a functions that 
take multiple parameter can easily be written as a funtion that takes a pair a 
function that takes pairs of pair. For instance, a function $f(x, y, z)$ and be 
written as $\lambda$ : $(T \times T) \times T \longrightarrow T$. 

\par
The basic addition and subtraction are available for integer types, conjunction 
and disjunction are there for boolean types. These are written in an infix 
manner to form expressions. Other expression in the language are if-statements 
and while-loop for control flow, assignments statements, variables literal. The 
\textit{call} statement is use to invoke functions and finally skip signify an 
empty action.
  ...
  \begin{center}
    \begin{tabular} {c}
      ...
      \inference {\Gamma \vdash e_1: bool \\ \Gamma \vdash e_2: T & \Gamma \vdash e_3: T}
        ...
        e_2 \text{\textbf{ else }} e_3: T}[(\textbf{if else})] \text{ }
      ...
      \inference {\lambda_1 = T_1 \longrightarrow T_2 & \Gamma \vdash e_1: T_1}
        {\Gamma \vdash \text{\textbf{call}}(\lambda_1, e_1) : T_2}[(\textbf{call})]
      & \\
      ...
      \inference {\Gamma \vdash e : \Pi x : A. P(x) & \Gamma \vdash e' : A} 
        {\Gamma \vdash (e, e') : [e'/x]P(x)}[($\Pi$ E)] \text{ }
    \end{tabular}
  \end{center}
\caption{Typing Rules for Simple-$\Pi$}
\end{figure}
