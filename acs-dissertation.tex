...
\section{Summary}
In this chapter we constructed an imperative programming language with native 
support for refinement types. We distinguished our system apart from the exisiting 
work by basing the underlying calculus on a simple ``\textit{WHILE}'' language 
...
\par
Throughout the chapter we demonstrated how type checking refinements type 
statically is undecidable in our system, we drew parallels by comparing 
mechanics of established languages such as downcasting in Java where the 
validity cannot be checked at compile time. Adopting a similar strategy, we 
presented a form of hybrid type checking that overcomes this limitation, the 
type checker verifies as much as possible statically and defers more complex 
verification to be done dynamically. 
...
types in the mutable environment. Our first approach is based on the 
idea of immutable variables and forbidding mutation on variables that is associated 
with the refinement predicate. The second approach utilises closures and makes 
a copy of the state at the time of type declaration, type checking will 
use the values in the copied state. 
...
Java and C/C++. Concluding the chapter, we address these ambiguities by 
