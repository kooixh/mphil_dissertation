...
\par
Using the judgement rule we can easily define the first few typing rules for base 
...
\begin{figure}[H]
  \begin{center}
    \begin{tabular} {c}
      ...
      \inference {\Gamma \vdash e_1: bool \\ \Gamma \vdash_{C} C_1: void & \Gamma \vdash_{C} C_2: void}
        {\Gamma \vdash_{C} \text{\textbf{if }} e_1 \text{\textbf{ then }} 
        C_1 \text{\textbf{ else }} C_2: void}[(If-Else)] & \\
      \inference {\Gamma \vdash e_1: bool & \Gamma \vdash_{C} C_1: void}
        {\Gamma \vdash_{C} \text{\textbf{while }} e_1 \text{\textbf{ do }} C_1 : void} [(While-Do)] \text{ }
      \inference {\Gamma \vdash_{C} C_0: void & \Gamma \vdash_{C} C_1 : void} 
...
\begin{figure}[H]
  \begin{center}
    \begin{tabular} {c}
      \inference{fts((x_0,x_1,\text{ }...\text{ },x_n) \longrightarrow C;e) = T_1,\text{ }...\text{ },T_n \longrightarrow T_0 \\ 
      ...
      & \\
      \inference{fts((x_0,x_1,\text{ }...\text{ },x_n) \longrightarrow \textbf{begin }D;C;e\textbf{ end}) = T_1,\text{ }...\text{ },T_n \longrightarrow T_0 \\ 
      \Gamma \vdash x_0 : T_1 & \Gamma \vdash x_1 : T_1 & ... & \Gamma \vdash x_n : T_n \\
...
\par
The typing judgement behaves similarly to our initial judgement, the only 
...
snapshotting pointers using a deep copy method. We observe that pointers in 
...
and store them in local variables, for example, if a 
refinement type takes the form $\{\upsilon \text{ }|\text{ }\upsilon < !!!p\}$, 
then the potential type system would snapshot the value of $p$ and rewrite the type as 
$\{\upsilon \text{ }|\text{ }\upsilon < p_{val}\}$ where $p_{val}$ is a local variables 
in the Type Closure that has the value of dereferecing the multi-level pointer $p$.
...
groups, those focusing on the theoretical aspects of refinement types in 
imperative languages and those relating refinement types to mainstream 
programming. 
...
conclude our research by extending these languages with two new variants Simple-$R^{*}_{I}$ and 
...
address this problem by adapting the concept of multi-level pointers to 
...
\paragraph{Advanced Language Features} One could extend Simple-$R$ 
