...
\begin{figure}[H]
  \begin{center}
    \begin{tabular}{c}
      \inference {\sigma' = \sigma \uplus \{x \mapsto v\} & \rho' = push(\rho, \langle \text{\textbf{skip}}, \sigma \rangle)} 
      {\langle \text{\textbf{let }}x = v \text{\textbf{ in }} e_1 \text{\textbf{ end}}, \sigma, \rho \rangle 
      \longrightarrow \langle e_1;\textbf{return }nil, \sigma'', \rho'\rangle} [(\textbf{let in} values)] 
      & \\
      \inference {\sigma' = \sigma \uplus \{x \mapsto v\} & \rho' = push(\rho, \langle e_2, \sigma \rangle)} 
      {\langle \text{\textbf{let }}x = v \text{\textbf{ in }} e_1 \text{\textbf{ end}};e_2, \sigma, \rho \rangle 
      \longrightarrow \langle e_1;\textbf{return }nil, \sigma'', \rho' \rangle} [(\textbf{let in} values contd)] 
      & \\
      \inference {\langle e_1, \sigma \rangle \longrightarrow \langle e_1', \sigma'\rangle} 
      {\langle \text{\textbf{let }}x = e_1 \text{\textbf{ in }} e_2 \text{\textbf{ end}}, \sigma, \rho \rangle 
      \longrightarrow \langle \text{\textbf{let }}x = e_1' \text{\textbf{ in }} e_2\text{\textbf{ end}}, \sigma', \rho'\rangle} 
      [(\textbf{let in} $expr$)] 
      &\\
      \inference {\langle e_2, \sigma' \rangle = top(\rho) & \rho' = pop(\rho)} {\langle \textbf{return } e_1, 
      \sigma, \rho \rangle \longrightarrow \langle e_1;e_2, \sigma', \rho' \rangle} [(\textbf{return} expr)]
      &\\
      \inference {\langle e_2, \sigma' \rangle = top(\rho) & \rho' = pop(\rho)} {\langle \textbf{return } nil, 
  ...
  \begin{center}
    \begin{tabular} {c}
      \inference {((x_1..x_n), e_{body}) = \textit{fns}(f) 
        \\ \sigma' = \{x_1 \mapsto v_1 ... x_n \mapsto v_n\} & \rho' = push(\rho, \langle \textbf{skip}, \sigma) \rangle}
      {\langle f(v_1...v_n), \sigma, \rho' \rangle \longrightarrow \langle e_{body}, \sigma', \rho' \rangle} [(function call vals)]
      & \\
      \inference {((x_1..x_n), e_{body}) = \textit{fns}(f) 
        \\ \sigma' = \{x_1 \mapsto v_1 ... x_n \mapsto v_n\} & \rho' = push(\rho, \langle e_1, \sigma) \rangle}
      {\langle f(v_1...v_n);e_1, \sigma, \rho' \rangle \longrightarrow \langle e_{body}, \sigma', \rho' \rangle} [(function call vals)]
      & \\
      \inference {\langle e_1, \sigma \rangle \longrightarrow \langle e_1, \sigma', \rho \rangle &...& \langle e_n, \sigma' \rangle \longrightarrow \langle e_n', \sigma'' \rangle}
...
\begin{figure}[H]
  \begin{center}
    ...
    \end{tabular}
  \end{center}
  \caption{Hoare Logic for Immutable Variable}
\end{figure}
...
Here instead, we will define the Hoare Logic that specify the behaviour. If 
$\Pi x : A. P(x)$ is a dependent type that depends on $x$ of type $A$ then $P \in $ 
\textit{dfns}, \textit{dfns} behaves like \textit{fns}, 
holding the signature of all dependent type functions.
...
\begin{figure}[H]
  \begin{center}
    \begin{tabular} {c}
      \inference {((x_1), e_{body}) = \textit{dfns(P)} 
      \\ \sigma' = \{x_1 \mapsto v\} & \rho' = push(\rho, \langle e_1, \sigma, \tau \rangle)}
      {\langle\textbf{typedef } x = \Pi v : A.P(v);e_1, 
      \sigma, \rho, \tau \rangle \longrightarrow 
        \langle \textbf{typedef } x = e_{body}, \sigma', \rho', \tau \rangle}[(\textbf{typedef}) $\Pi$]  
      & \\
      \inference {\langle e_1, \sigma, \rho, \tau \rangle \longrightarrow \langle e_1', \sigma', \rho', \tau' \rangle}
      {\langle \textbf{typedef }x = e_1, \sigma, \rho, \tau \rangle \longrightarrow 
        \langle \textbf{typedef }x = e_1', \sigma', \rho', \tau' \rangle}[(\textbf{typedef}) $\Pi$ eval] 
      & \\ 
      \inference {\langle e_1, \sigma', \tau' \rangle = top(\rho) & \rho' = pop(\rho)}
      {\langle \textbf{typedef } x = \textbf{return }T, \sigma, \rho, \tau \rangle \longrightarrow 
        \langle e_1, \sigma', \rho', \tau' \uplus {x \mapsto T} \rangle}[(\textbf{typedef}) $\Pi$ return] 
    \end{tabular}
  \end{center}
...
\par 
The three rules above formalise the semantics for dependent type definitions in 
...
We first define the base types for heap address and array values. 
\begin{figure}[H]
  \begin{center}
    \begin{tabular} {c}
...
\begin{figure}[H]
  \begin{center}
    \begin{tabular} {c}
      ...
      & \\
      \inference {\Gamma \vdash e_1 : T\textit{ ref}} 
...
begin by defining the SOS rules for pointers.
\begin{figure}[H]
  \begin{center}
    \begin{tabular} {c}
      \inference {v = \Delta(h_1)}
      ...
      \inference {h \notin \textbf{dom}(\Delta) & \Delta' = \Delta 
        \uplus \{h \mapsto nil\}}
        {\langle \textbf{new}(T), \sigma, \rho, \tau, \Delta \rangle \longrightarrow \langle v, 
...
\begin{figure}[H]
  \begin{center}
    \begin{tabular} {c}
      ...
      & \\
      \inference {v_0 = (h_0,h_1...h_n) & v_1 >= 0 & v_1 <= n & \Delta(h_{v_1}) := v_2} 
