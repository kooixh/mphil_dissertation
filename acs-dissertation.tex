...
commonly associated with these type systems, we review the Agda, Idris and Haskell 
...
the reason being its paradigm, Agda is \textit{purely functional} \cite{purelyFP}, 
meaning that all functions are pure (i.e. not relying on the program state or other mutable 
...
appeared in 2007 and bears similarity with Agda, both in terms of paradigm 
and type system. However they differ in one crucial way, Idris was designed to 
emphasise general purpose programming rather than theorem proving. It provides 
interoperability with systems libraries and C programs, 
as well as language constructs for domain specific language 
implementation \cite{gpIdris}. However, Idris remain predominantly a research tool. 
...
where one is allowed to take the address of a local variable, in our 
...
value. The command $!e_1 := e_2$ changes the value at heap address after evaluating 
$e_1$ to the value after evaluating $e_2$.
...
\paragraph{Heap}
Finally, we define a heap context $\Delta$ as a mapping from heap addresses to values.  
...
We define the rules for pointers in figure \ref{fig:ref_type_rules}. 
...
\begin{figure}[H]
  ...
  \label{fig:ref_type_rules}
\end{figure}


\subsubsection{Operational Semantics}
Lastly, we define the operational semantics for the new expression forms in figure 
\ref{fig:sos_pointers}. Pointers are allocated using the \textbf{new} expression which takes a type $T$ 
and a value $v$, the evaluation rules allocate a new heap address $h$ that is 
...
modifying the underlying heap address and the other is that no aliases 
should be able to modify the address indirectly.
...
new addition. In addition to all free variables being immutable, a new rule states 
...
\par
The example program shown previously in figure \ref{fig:amb_refinement_pointers} 
is now an invalid program. To make it valid we adapt the newly defined rules 
...
snapshotting pointers using a \emph{deep copy} method and storing these in closures. 
We observe that pointers in 
...
\par
Using a deep copy approach improves flexibility as one does not need to declare 
pointers to immutable in order to use them in refinement types. Due to the 
limits of time, we propose this as an interesting idea and leave the formal 
...
compared to those using Monads such as Ynot.
