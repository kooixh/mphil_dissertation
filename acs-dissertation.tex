...
our language. The introduction of arrays ties closely with pointers and aliases, 
the behaviours of these two addition will be discussed and specified in this 
section.

\subsection{Introduction of Heap Memory Allocation}
In order to be able to accomodate arrays in a programming language we will have 
to deal with allocating memory using \textit{Heaps} \cite{heapVsStack}. 
Currently in the Simple-$\Pi$ language we have not explicitly discussed our 
memory model but it is obvious it is using the stack memory. As we are dealing 
with dependent types and arrays are likely to be dynamic, we decided to use heap 
memory to handle arrays. 
\par
In this subsection we will extend the syntax to include the array data types, 
expressions on arrays and the heap memory structure. 
  ...
  \begin{center}
    \begin{tabular} {l l l}
      ...
      \textit{Expressions} & $e$& $:$ $...\text{ }|\text{ }e_1[e_2]\text{ }
        ...
        & & \; $|\text{ }\textbf{newarray}(e_1, e_2)\text{ }|\text{ }!e_1\text{ }|\text{ \textbf{new}}(T)$ \\
     \textit{Values} & $vals$& $:$ $...\text{ }|\text{ }h\text{ }|\text{ }(h_0,h_1...h_n)$ \\
     \textit{Heap Address} & $\Delta$& $:$ $h \in \{h_1,h_2, h_3...\}$ \\
    \end{tabular}
  \end{center}
  \caption{Extended Syntax for Simple-$\Pi$}
\end{figure}
