...
to the problem. Dependent types allows the programmer to create types whose 
definition depends on a value. A type system that provides such refined 
control over the values it can take unlocks possibility that are previously 
unavailable such as domain-specific type checking at compile time.
...
\section{Motivation}
\par
As much as we have studied about type systems, dependent types remains 
uncommon in the industry. While the theory of dependent types has been 
...
programmers have attempted to bring refinement types to the mainstream, their 
work remains niche. 
...
\subsection{Basic Definition of Dependent Types}
...
\begin{figure}[H]
  \begin{center}
    \begin{tabular}{c}
      ...
      \rangle \longrightarrow \langle \sigma', \tau \rangle}
      {\langle\textbf{begin var }x : \{\upsilon : T\text{ }|\text{ }v\} = v_0; C_1 \textbf{ end}, \sigma, \tau, \mathcal{R} \rangle 
...
\begin{figure}[H]
  \begin{center}
    \begin{tabular} {l l l}
      ...
      \textit{Expressions} & $e$ & $:$ $...\text{ }|\text{ }!e_1\text{ }|\textbf{ new}(T, v)$\\
      \textit{Commands} & $C$ & $:$ $...$ $|$ $!h := e$ \\
      \textit{Values} & $vals$& $:$ $...\text{ }|\text{ }h \in \{h_1,h_2, h_3...\} $ $|$ $nil$ \\
...
assumme that \textit{Automatic Garbage Collection} \cite{GC} 
is present in the language. 

\paragraph{Commands}
We add a new command that dereference a heap address and modifies its underlying 
value. The command $!h := e$ changes the value at heap address $h_1$ to the 
...
A $nil$ reference is treated as a standalone 
value and not a dedicated heap address as it is the case in C/C++ where a 
special address is used for NULL pointers. 
...
As it is the case with many languages, the $nil$ value is a reference type and a 
heap address is a reference type. 
...
\begin{figure}[H]
  \begin{center}
    \begin{tabular} {c}
      ...
      \text{ }
      \inference {\Gamma \vdash v : T}{\Gamma \vdash \textbf{new}(T, v) : T\textit{ ref}}[(\textbf{new} ref)]
      & \\
      \inference {\Gamma \vdash e : T & \Gamma \vdash h : T\textit{ ref}} {\Gamma \vdash_{C}\text{ }!h := e : void}
    \end{tabular}
  \end{center}
...
\ref{fig:sos_pointers}. Note that in our semantics we ommited the reduction 
rules to reduce an expression into a heap address value as the rules are 
trivial. 
...
\begin{figure}[H]
  \begin{center}
    \begin{tabular} {c}
      ...
      \sigma, \tau, \Delta \rangle} [(Pointer deref 1)]
      & \\
      \inference {h_1 \in \textbf{dom}(\Delta) & h_1 \neq nil & \Delta' = \Delta[h_1 \mapsto v_1]}
        ...
        \sigma, \tau, \Delta' \rangle} [(Pointer deref 2)]
      & \\
...
not currently present, assigns it the value $v$ and return the address. The 
value can be retrieve and modify through dereferencing, shown in the first two 
rules. A $nil$ reference is not allocated in the state $\Delta$ and cannot be 
deference. 
...
arises once again, the relation between pointers and refinement types is 
unclear. Throughout the text we learnt that we can avoid ambiguous behaviour 
between mutation and refinement by using immutability or closures. This appears 
...
\begin{figure} [H]
  \begin{tabs}{1cm,2cm}
    ...
    $\textbf{begin var }$x\text{ }:\text{ } int\textit{ ref} = y;$\\
    \tab\textbf{begin typedef }$BoundedIntY$ = $\{i : int\text{ }|\text{ }i <\text{ }!y\}$; \\ 
    \tab\tab\textbf{begin var }$z\text{ }:\text{ }BoundedIntY = 6;!x := 5$\textbf { end} \\
...
initialised with the value 10. We then define another variable $x$ that acts as 
its alias, i.e. both $x$ and $y$ are different variables but points to the same 
address. The expression $!x := 5$ indirectly modified the definition of the type 
$BoundedIntY$, making $z$ ill-typed. 
...
modify the heap address and have he changes reflect within the closure.
...
\textbf{newconst}$(T, v)$ which allocates a constant value and return its 
pointer. 
...
typing judgement for this new expression. We define the operational semantics 
for the new expression in figure \ref{fig:sos_const_ptr}. Additionally, we 
restrict the ability to modify a the value of a constant heap address.
...
new addition. In addition to all variables being immutable, a new predicate 
requires that all pointer variables in the 
expression must to be pointing to a constant. This ensures that the value pointed 
...
Our second approach uses closures to handle refinement type, the approach 
can easily be modified to work with pointers with a 
...
as pointers and aliasing. We observe how introducing new form of 
variables brings new types of mutation into the language and therefore 
requires us to adapt new strategies in our language specification. We proposed a  
new requirements that ensures pointers are pointing to immutable locations if used 
in a refinement predicate. Finally, we incorporate these new requirements 
into our language by extending previously defined semantics. 

\section{Summary}
Throughout this chapter we described an imperative programming langauge 
with refinement types support that is heavily based on C/C++. We showed how 
statically verifying refinement types is hard to achieve because of the presence 
of mutation and variables whose values are only available at runtime. We 
overcome this by adopting a hybrid type checking approach that type checks as 
much as possible statically but defers complicated checks to runtime. However, 
we observed that runtime type checking has some ambiguities because of the 
presence of mutation, we further specified our semantics by proposing two ideas 
that handles mutation in refinement types. Our first approach is based on the 
idea of immutable variable and restricting mutation on variables that is associated 
with refinement types. The second approach utilises closures which makes a copy of 
the state at the time of type declaration, type checking will 
use the values in the copied state. Finally, we showed how adding 
dynamically allocated memory will lead to our definition underspecified, we 
conclude the chapter by addressing these new ambiguities with newly defined 
semantics. 
