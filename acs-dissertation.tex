...
different language constructs. In this chapter, we introduce three key concepts, 
immutability, closures and hybrid type checking. We discuss these concepts 
in depth, identifying how they relate to refinement types and how they 
...
\subsection{Static vs Dynamic Type Checking}
...
\subsection{Example of Hybrid Type Checking in Java} 
...
formalise the behaviour. To increase readability, in future text when we 
introduced new form of \textit{Declarations}, we will only 
be formalising the rules where only a single declaration is present. The 
concetenation rules which can be derived easily using the same semantics in 
figure \ref{scoping_rules} are omitted.
