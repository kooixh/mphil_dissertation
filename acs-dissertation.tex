...
\begin{figure}[H]
  \begin{center}
    \begin{tabular} {c}
      ...
      \inference {\Gamma \vdash x : T_1 \longrightarrow T_2  & \Gamma \vdash e: T_1}
        {\Gamma \vdash \text{\textbf{call}}(x, e) : T_2}[(\textbf{call})]
    \end{tabular}
  \end{center}
...
to express substituion. A substitution $\sigma$ is a mapping from variables to 
...
\begin{figure}[H]
  \begin{center}
    \begin{tabular} {c}
      $\langle \textbf{call}(\lambda void.e, v), s \rangle \longrightarrow \langle e, s \rangle$ (\textbf{call} with void)
      & \\
      $\langle \textbf{call}(\lambda (x,x_1...x_n).e, (v_1,v_2...v_n)), s \rangle \longrightarrow \langle [x_1/v_1...x_n/v_n]e, s \rangle$ (\textbf{call} with params)
    \end{tabular}
  \end{center}
...
represent the type variable map. 
...
\begin{figure}[H]
  \begin{center}
    \begin{tabular} {l l l}
      ...
      \textit{Value Variable Context} & $\Gamma$& $:$ $vars \mapsto 
      vals$ \\
...
\par
We then define the operational semantics, let $t$ range over all Type Variable 
Context: 
...
\begin{figure}[H]
  \begin{center}
    ...
    \begin{tabular}{c}
      $\{a \notin \text{\textbf{dom}}(\Gamma)\}a = \Pi x. P(x)\{consts = consts' \cup \{x\} 
      \wedge \Upsilon(a) = \Pi x. P(x)\}$ & \\  
      $\{x \notin consts\}x := e\{\Gamma(x) = e\}$
    \end{tabular}
  \end{center}
...
\begin{figure}[H]
  \begin{center}
    \small$\{x \notin \text{\textbf{dom}}(\Gamma) \wedge \alpha = \Gamma(x)\}
