...
uses the notation $-$, we define the operator to operate on a state and a set of 
$vars$. $\sigma - \{x\}$ states that remove all the variables in the set 
...
\begin{figure}[H]
  \begin{center}
   \begin {tabular} {c} 
     ...
     \\ \sigma' = \sigma[\beta \mapsto \alpha : \Pi v. P(v)\text{ }|\text{ }\beta \in \textbf{dom}(\sigma) \wedge \sigma(\beta) = \alpha : \Pi x. P(x)]
     \\ \tau' = \tau[\beta \mapsto \Pi v. P(v)\text{ }|\text{ }\beta \in \textbf{dom}(\tau) \wedge \tau(\beta) = \Pi x. P(x)]} 
...
\par
We define $\phi$ as a local function $T \longrightarrow \{vars\}$, defined as 

\begin{center}
  $\phi(T) = \{x\text{ }|\text{ }x \in \sigma \wedge \sigma(x) = \alpha : T\}$  
\end{center}

\par
Given a type $T$, the function $\phi$ returns the set of all variables in the 
state $\sigma$ that has type $T$.
...
\begin{figure}[H]
  \begin{center}
    \footnotesize$\forall \gamma \in \mathcal{P}(\sigma)\{\gamma = \phi(\Pi x : A.P(x)) \wedge P[e/x]\}x:= e\{P \wedge 
    \forall x \in \gamma\text{ }s.t.\text{ }\sigma(x) = \alpha : \Pi e : A. P(e)\}$ 
    \normalsize
  \end{center}
  \caption{Dynamic Dependedent Type Reassignment Rule}
\end{figure}
  ...
  \begin{center}
    \begin{tabular} {c}
      ...
      \inference {h \notin \textbf{dom}(\Delta) & \Delta' = \Delta 
        \uplus \{h \mapsto nil\}}
        {\langle \textbf{new}(T), \sigma, \tau, \Delta \rangle \longrightarrow \langle h, 
...
\begin{figure} [H]
  \begin{tabs}{1cm,2cm}
    \textbf{let const }$y\text{ }:\text{ } int \textit{ ref} = \textbf{new}(int)$\textbf{ in } !y := 10; \\
...
to apply the second rule. In addition to $x$ being a constant and cannot be 
reassigned to a different head address, the underlying memory location held by $x$ 
must not be rewriteable. 
