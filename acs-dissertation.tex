...
\par
We now define the rules for type checking an expression against a declared type, 
as a declared type is only ever a refinement type, we have established 
that they cannot be type checked statically. However, we must 
still validate the \emph{Refine-Base-US} rule. 
\begin{center}
  \begin{tabular} {c}
